...
class Workflow(object):
    ...
    def set_state(self, state, state_info=None, recursive=False):
        ...
        if states.is_valid_transition(cur_state, state):
            ...
            if state != cur_state:
                kfk_trace.log(kfk_etypes.wf_parse(cur_state, state), None, state,
                            self.wf_ex.workflow_id, self.wf_ex.id, None, None,
                            self.wf_ex.input, self.wf_ex.output, triggered_by=None)
        else:
